...
struct Neighbor {
  ...
  Neighbor() : score(0.0f), sequence_number(0) {}
  Neighbor(const InternedStringPtr& external_id, float score)
      : external_id(external_id), score(score), sequence_number(0) {}
  Neighbor(const InternedStringPtr& external_id, float score,
           std::optional<RecordsMap>&& attribute_contents)
  ...
  friend std::ostream& operator<<(std::ostream& os, const Neighbor& n) {
    os << "Key: " << n.external_id->Str() << " Score: " << n.score
