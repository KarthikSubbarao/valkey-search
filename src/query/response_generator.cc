...
void ProcessNeighborsForReply(
  ...
  vmsdk::ValkeySelectDbGuard select_db_guard(ctx, parameters.db_num);
  // Fast path eligibility: hash content via ScanKeyRawBorrowed -> borrowed
  // ptr+len refs -> ReplyWithStringBuffer (zero-copy). Applies to plain,
  // vector (score prefix added at reply time), and RETURN-subset queries.
  // Also SORTBY: we pin the sort field and order by it, then emit the
  // (zero-copy) content -- only the sort key is inspected, never copied.
  // Excludes JSON (values aren't borrowable hash sds) and any computed/
  // transformed reply value (FT.AGGREGATE).
  // Raw borrowed-content fast path. Requires a core that exports
  // VM_ScanKeyRawBorrowed (enforced at module load; no runtime fallback).
  ...
  // doc). When SORTBY is present we ALSO pin the sort field so ApplySorting
  // can order by it.
  ...
  if (raw_eligible && raw_has_return) {
    ...
    if (parameters.sortby_parameter.has_value()) {
      auto id = parameters.index_schema->GetIdentifier(
          parameters.sortby_parameter->field);
      raw_sortby_identifier =
          id.ok() ? *id : parameters.sortby_parameter->field;
  ...
  for (auto &neighbor : neighbors) {
    ...
    if (raw_eligible) {
      ...
      auto raw = FetchAllHashFieldsRawBorrowed(
